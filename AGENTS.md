...
- `$cosh-before-push` 只允许用户手动触发。只有用户明确点名 `$cosh-before-push` 或明确要求“使用/调用 before-push skill”时才运行；普通的 push、推送、同步到远端、“推送吧”“直接推”“提交并推送”等请求不得自动触发。
- 用户未手动触发 `$cosh-before-push` 时，按普通 Git 流程处理 push，不主动补做本 skill 的 CR，也不要求额外确认。
- 用户手动触发后，不得把 `$cosh-before-push` 理解为 Git hook、仓库脚本或 bytedcli 流程，也不得先执行 `git push` 再补做评审。
- 门禁必须完成 Coco runner 与 Codex reviewer 两路独立 CR，并由主执行者汇总结论；两路都综合检查合理性、正确性、安全性及注释实现一致性。
...
- 报告后的最终确认是同一任务、同一已评审快照的续接状态，视为已满足 `$cosh-before-push`；不得再次触发 CR。后续只有用户再次手动调用本 skill 才启动新的评审。
- 用户可以人工覆盖门禁。人工覆盖只表示接受已列问题并继续普通 `git push`，不表示使用 Git `--force`，也不表示 CR 通过。
