...
BYTED_DIR = ROOT / "skills" / "cosh-byted-review-planner"


class BytedNativeReviewPlannerSkillTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.workflow = (
            BYTED_DIR / "references" / "byted-workflow.md"
        ...
        cls.prompts = (
            BYTED_DIR / "references" / "spec-plan-prompts.md"
        ).read_text(encoding="utf-8")

    def test_identity_uses_byted_native_flow(self) -> None:
        self.assertIn("name: cosh-byted-review-planner", self.skill)
        self.assertIn("字节", self.skill)
        self.assertIn("字节原生", self.skill)
        self.assertIn("$cosh-byted-review-planner", self.metadata)

    def test_native_prompts_lock_spec_and_plan_precision(self) -> None:
        self.assertIn("规格生成 Prompt", self.prompts)
        self.assertIn("实施计划生成 Prompt", self.prompts)
        self.assertIn("文件、符号、变量和接口", self.prompts)
        self.assertIn("### Task N:", self.prompts)
        self.assertIn("- Test:", self.prompts)
        self.assertIn("references/spec-plan-prompts.md", self.skill)

    def test_required_references_exist(self) -> None:
        for relative in (
            "references/byted-workflow.md",
            ...
            "references/spec-plan-prompts.md",
        ):
    ...
    def test_skill_starts_and_opens_dashboard_before_ai_spec(self) -> None:
        ...
        self.assertIn("<skill-root>/scripts/serve_byted_dashboard.py", self.skill)
        self.assertIn("--port 57171 --open", dashboard)
        self.assertIn("<skill-root>/scripts/serve_byted_dashboard.py", dashboard)
    ...
    def test_archive_is_local_gitignored_and_distills_rules(self) -> None:
        self.assertIn(".byted-dev/archive/", self.workflow)
