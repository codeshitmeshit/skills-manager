...
SKILL_DIR = ROOT / "skills" / "cosh-byted-review-planner"
WORKFLOW_PATH = SKILL_DIR / "scripts" / "workflow_state.py"
TASK_CONTROL_PATH = SKILL_DIR / "scripts" / "task_control.py"
SERVER_PATH = SKILL_DIR / "scripts" / "serve_byted_dashboard.py"
...
SERVER = load_module("byted_native_dashboard", SERVER_PATH)
...
class BytedNativeWorkflowStateTest(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_dir = tempfile.TemporaryDirectory()
        self.root = pathlib.Path(self.temp_dir.name)
        self.work_id = "optimize-order-risk-check"
        self.work = self.root / ".byted-dev" / "work" / self.work_id
        (self.work / "reviews").mkdir(parents=True)
        (self.work / "evidence").mkdir()
        (self.root / "docs" / "byted-dev" / "specs").mkdir(parents=True)
        (self.root / "docs" / "byted-dev" / "plans").mkdir(parents=True)
    ...
    def write_knowledge_gate(self, status: str = "passed", **extra) -> None:
        sources = []
        for role in sorted(set().union(*WORKFLOW.REQUIRED_KNOWLEDGE.values())):
            path = self.work / "knowledge" / f"{role}.md"
            path.parent.mkdir(exist_ok=True)
            path.write_text(f"Knowledge {role}", encoding="utf-8")
            sources.append({"role": role, "path": str(path.relative_to(self.root)), "sha256": self.sha256(f"Knowledge {role}")})
        payload = {
            **self.current_binding(),
            ...
            "version": "1.2.3",
            "sources": sources,
            "reviewer_sources": {k: sorted(v) for k, v in WORKFLOW.REQUIRED_KNOWLEDGE.items()},
            "loaded_at": "2026-08-02T10:05:00+08:00",
    ...
    def write_task_evidence(self, task_number: int, snapshot: str | None = None) -> None:
        snapshot = snapshot or TASKS.current_snapshot_sha(self.root)
        for prefix in ("remote-ut", "cr"):
            self.write_json(
                self.work / "evidence" / f"{prefix}-task{task_number}.json",
                {
                    "status": "passed",
                    "code_sha": snapshot,
                    "prepare_only": False, "exit_code": 0,
                    "result_dir": "remote/report", "scope": "affected packages",
                    "meta": {"remote": {"run_success": True}},
                    "summary": {"has_failures": False, "failed_packages": 0, "failed_tests": 0},
                    "reviewer": "reviewer-1", "review_summary": "当前 diff 已审查", "blocking_findings": [],
    ...
    def test_knowledge_gate_rejects_missing_changed_and_escaping_sources(self) -> None:
        for path in ("missing.md", "../outside.md"):
            self.write_knowledge_gate(sources=[{"role": "stability_spec", "path": path, "sha256": "invalid"}])
            self.assertEqual(WORKFLOW.build_status(self.root, self.work_id)["stages"]["knowledge_gate"]["status"], "blocked")
        self.write_knowledge_gate()
        (self.work / "knowledge" / "stability_spec.md").write_text("changed")
        self.assertEqual(WORKFLOW.build_status(self.root, self.work_id)["stages"]["knowledge_gate"]["status"], "blocked")

    def test_staged_other_work_metadata_is_out_of_scope(self) -> None:
        self.initialize_git()
        self.stage_file("internal/order/risk.go")
        self.stage_file(".byted-dev/work/other/state.json")
        self.assertEqual(TASKS.validate_task_scope(self.root, {"allowed_files": ["internal/order/risk.go"]}), [".byted-dev/work/other/state.json"])

    def test_task_evidence_rejects_contradictory_results(self) -> None:
        self.initialize_git()
        self.write_task_evidence(1)
        snapshot = TASKS.current_snapshot_sha(self.root)
        for prefix, field, value in (("remote-ut", "prepare_only", True), ("remote-ut", "summary", {"has_failures": True}), ("cr", "blocking_findings", ["open issue"])):
            self.write_task_evidence(1)
            path = self.work / "evidence" / f"{prefix}-task1.json"
            evidence = json.loads(path.read_text())
            evidence[field] = value
            self.write_json(path, evidence)
            with self.assertRaises(TASKS.TaskControlConflict):
                TASKS._validate_task_evidence(self.work, 1, snapshot)
            self.assertFalse(TASKS._evidence_matches_snapshot(self.work, prefix, 1, snapshot))

    def test_mode_switch_rejects_uncommitted_task(self) -> None:
        self.initialize_git()
        self.stage_file("internal/order/risk.go")
        with self.assertRaisesRegex(SERVER.workflow_state.DashboardConflict, "未提交"):
            SERVER._set_mode(self.root, self.work_id, WORKFLOW.build_status(self.root, self.work_id)["version"], "continuous")

    def test_chat_mode_requires_explicit_intent_and_records_message(self) -> None:
        payload = {"work": self.work_id, "action": "set-mode", "mode": "continuous", "origin": "chat", "expected_version": WORKFLOW.build_status(self.root, self.work_id)["version"], "idempotency_key": "chat-mode"}
        with self.assertRaisesRegex(SERVER.workflow_state.DashboardConflict, "明确意图"):
            SERVER.apply_control(self.root, payload)
        payload.update(user_intent="继续", message_id="user-message-1")
        with self.assertRaisesRegex(SERVER.workflow_state.DashboardConflict, "明确意图"):
            SERVER.apply_control(self.root, payload)
        payload.update(user_intent="切换为连续推进", message_id="user-message-1")
        SERVER.apply_control(self.root, payload)
        control = json.loads((self.work / "control.json").read_text())
        self.assertEqual(control["authorization_history"][-1]["message_id"], "user-message-1")

    def test_automation_cannot_select_continuous_mode(self) -> None:
        payload = {"work": self.work_id, "action": "set-mode", "mode": "continuous", "origin": "automation", "expected_version": WORKFLOW.build_status(self.root, self.work_id)["version"], "idempotency_key": "auto-mode"}
        with self.assertRaisesRegex(SERVER.workflow_state.DashboardConflict, "代替用户"):
            SERVER.apply_control(self.root, payload)
        self.assertEqual(json.loads((self.work / "workflow.json").read_text())["mode"], "single")

    def test_shared_file_requires_snapshot_bound_scope_review(self) -> None:
        tasks = [{"number": 1, "allowed_files": ["shared.go"]}, {"number": 2, "allowed_files": ["shared.go"]}]
        self.assertTrue(TASKS._shared_scope_errors(self.work, tasks[0], tasks, "snapshot"))
        self.write_json(self.work / "evidence" / "scope-task1.json", {"status": "passed", "task": 1, "code_sha": "snapshot", "out_of_scope_changes": [], "reviewer": "reviewer", "review_summary": "仅修改当前符号"})
        self.assertFalse(TASKS._shared_scope_errors(self.work, tasks[0], tasks, "snapshot"))
        self.assertTrue(TASKS._shared_scope_errors(self.work, tasks[0], tasks, "changed"))

    def test_list_works_returns_multiple_development_works(self) -> None:
        another = self.root / ".byted-dev" / "work" / "add-audit-log"
        another.mkdir()
        self.write_json(
            another / "source.json",
            {
    ...
    def test_hammer_engine_blocks_the_complete_byted_workflow(self) -> None:
...
"""
        ...
        tasks = TASKS.parse_byted_plan(plan)
        ...
        self.assertIn("exampleSymbol", tasks[0]["symbols"][0])
        self.assertIn("exampleVariable", tasks[0]["variables"][0])
        self.assertIn("shouldSkip", tasks[0]["interfaces"][0])
        self.assertEqual(tasks[0]["acceptance_criteria"], ["行为符合规格"])
...
"""
        ...
        self.write_task_evidence(1, snapshot)
        version = WORKFLOW.build_status(self.root, self.work_id)["version"]
...
"""
        ...
        result = SERVER.apply_control(self.root, {
            "work": self.work_id, "action": "advance-auto", "origin": "automation",
            "expected_version": version, "expected_task": 1, "idempotency_key": "auto-task1",
            "commit_type": "feat", "summary": "增加内部重试判断",
        })
        self.assertEqual(result["tasks_done"], 1)
        self.assertTrue((self.work / "evidence" / "commit-task1.json").is_file())
...
"""
    ...
    def test_dashboard_cli_accepts_open_flag(self) -> None:
        with mock.patch.object(
            sys,
            "argv",
            ["serve_byted_dashboard.py", "--open", "--port", "57171"],
        ):
    ...
    def test_dashboard_cli_rejects_non_fixed_port(self) -> None:
        with mock.patch.object(
            sys,
            "argv",
            ["serve_byted_dashboard.py", "--port", "49999"],
        ):
    ...
    def test_dashboard_cli_uses_fixed_port_by_default(self) -> None:
        with mock.patch.object(sys, "argv", ["serve_byted_dashboard.py"]):
            args = SERVER.parse_args()
        self.assertEqual(args.port, 57171)
    ...
    def test_dashboard_opens_actual_bound_url(self) -> None:
        ...
        self.assertIn(
            "字节研发观察板: http://127.0.0.1:54321/", output.getvalue()
        )
    ...
    def test_document_endpoint_reads_only_selected_work_artifacts(self) -> None:
        server = self.start_server()
        relative = f".byted-dev/work/{self.work_id}/technical-design.md"
...
"""
    ...
    def test_push_evidence_triggers_archive_once(self) -> None:
        ...
        archives = list(
            (self.root / ".byted-dev" / "archive" / self.work_id).glob(
                "*-retrospective.md"
            )
        )
