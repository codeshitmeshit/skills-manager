...
description: 仅当用户明确点名 `$cosh-before-push` 或明确要求使用、调用 before-push skill 时使用；普通的 push、推送、同步到远端、“推送吧”“直接推”“提交并推送”等请求不得自动触发。手动触发后，在 git push 前完成 Coco 与 Codex 两路独立 CR；优先审查未推送 commits，仅当没有未推送 commit 时审查暂存区文件。完整报告后等待用户再次明确确认；不调用 AIME。
---
...
仅在用户手动触发本 skill 后，对未推送的本地 commits 做代码 CR；没有未推送 commit 时，改为对暂存区文件做 CR，并输出门禁结论。普通 `git push` 请求不得自动触发本 skill。手动触发后不得跳过、静默省略或先 push 后评审，也不得把本 skill 误解为 Git hook、仓库脚本或 bytedcli 流程。不要扩展成 Git 推送安全控制器：不管理 push URL、refspec、force、upstream、hook、认证或传输方式。用户手动触发只授权启动 CR；即使评审通过，也必须在完整报告之后等待用户再次明确确认，不能自动 push。
...
10. 只有用户在报告之后另行明确回复“推送”“继续”“同意”等确认，主流程才执行普通 `git push`，不增加其他推送前检查。该回复是同一任务、同一已评审快照的续接状态，视为已经完成本 skill，不得再次触发 CR；后续只有用户再次手动调用本 skill 才启动新的评审。
...
- 报告后的确认作为同一快照的续接状态，没有重复触发 CR；只有用户再次手动调用本 skill 才重新评审。
- 普通 push、推送或同步请求没有自动触发本 skill，也没有被本 skill 增加额外确认步骤。
