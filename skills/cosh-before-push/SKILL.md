...
description: 任何即将执行 git push 的请求都必须使用，包括“push”“推送”“推送吧”“直接推”“提交并推送”“同步到远端”等表达；也用于单独要求 push 前 CR、检查未推送 commits、暂存文件或判断代码是否适合推送。必须在 git push 之前完成 Coco、正确性、安全性、注释实现一致性四路独立 CR；优先审查未推送 commits，仅当没有未推送 commit 时审查暂存区文件。禁止先 push 后补评审；不调用 AIME，不管理具体 Git push 方式。
---
