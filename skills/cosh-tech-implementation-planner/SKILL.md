...
description: 根据技术方案和仓库现有代码，分析技术需求的最小改动实现流程，并生成详细的代码调用图和预期变更图。用户要求读取技术方案、定位相关代码、输出代码实现步骤、绘制 Mermaid 调用图/预期变更图、根据仓库调用关系生图、在真正改代码前先评审实现流程，或确认实现思路后制定开发 Plan 时使用。确认实现思路前只能读取和分析，不得修改仓库文件；用户确认实现思路后，必须先进入 Plan 模式或输出等价开发计划并等待确认，确认开发计划后才允许落库实现。
---
...
在真正改代码前，把技术方案中的具体实现落到仓库现有代码结构上，输出可评审的实现流程、详细调用图、预期变更图和最小改动方案。该 skill 只在用户两次确认后才进入代码落库：先确认实现思路，再确认开发 Plan。
...
6. 输出实现流程评审稿，不修改、创建、删除任何仓库文件。
7. 等待用户明确确认实现思路。用户没有确认前，不进入开发计划，也不落库。
8. 用户确认实现思路后，进入当前产品支持的 Plan 模式来总结开发计划；如果运行环境无法切换 Plan 模式，则输出等价的“开发 Plan”并明确等待确认。
9. 开发 Plan 必须说明修改文件、修改目的、实施顺序、验证方式和风险控制。
10. 等待用户明确确认开发 Plan。用户没有确认 Plan 前，不修改仓库文件。
11. 用户确认开发 Plan 后，才允许按计划进入代码实现；实现时继续遵守最小改动原则。
...
- 默认输出 Mermaid 源图，保证调用关系可审阅、可复制、可渲染。
- 当用户明确要求“生图”“图片版”“可视化图片”时，可以基于 Mermaid 和代码证据调用生图能力生成辅助图片；图片只作为阅读辅助，不能替代 Mermaid 源图、文件路径和代码证据。
- 生图提示词必须要求画面忠实呈现调用节点、边、分组和预期变更标注，不添加代码中不存在的模块、流程、风险、待确认项或测试项。
