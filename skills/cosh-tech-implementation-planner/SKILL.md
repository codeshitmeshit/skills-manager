...
1. 先判断实现需求是否足够明确。用户可能只给自然语言诉求而不是技术方案；此时先尝试理解业务意图和技术改动含义，再通过反问确认理解是否准确。若不明确，先澄清问题；澄清完成前不得生成 Mermaid、不得生图、不得制定 Plan、不得落库。
2. 读取用户提供的技术方案或自然语言诉求，提取本次需求目标、约束、预期行为、涉及模块和待实现点。
3. 如果用户没有给出明确方案文件或代码仓库位置，先用已有上下文和 `rg --files` 查找可能相关的方案文档；仍无法定位时，只询问必要的文件路径或关键词。
4. 读取仓库相关代码，优先用 `rg` 定位入口、调用链、类型定义、配置、已有相似实现和测试，不依赖猜测。
5. 对齐仓库现有风格，包括目录归属、命名方式、错误处理、日志、测试组织和已有辅助函数。
6. 基于实际代码证据生成现有调用图，并基于准备采用的最小改动方案生成预期变更图。
7. 基于 Mermaid 源图和代码证据调用生图能力，生成图片版可视化图。
8. 输出实现流程评审稿，不修改、创建、删除任何仓库文件。
9. 等待用户明确确认实现思路。用户没有确认前，不进入开发计划，也不落库。
10. 用户确认实现思路后，进入当前产品支持的 Plan 模式来总结开发计划；如果运行环境无法切换 Plan 模式，则输出等价的“开发 Plan”并明确等待确认。
11. 开发 Plan 必须说明修改文件、修改目的、实施顺序、验证方式和风险控制。
12. 等待用户明确确认开发 Plan。用户没有确认 Plan 前，不修改仓库文件。
13. 用户确认开发 Plan 后，才允许按计划进入代码实现；实现时继续遵守最小改动原则。

## 澄清规则

在读取代码和生成图之前，先确认实现需求是否足够明确。用户没有提供技术方案、只提供自然语言诉求时，不要要求用户先写技术方案；应先基于现有表述尝试归纳需求含义，再反问确认。若存在以下任一情况，必须先向用户澄清，不得直接进入实现流程分析：

- 不清楚要改哪个接口、方法、任务、事件、页面入口或配置入口。
- 不清楚本次改动的触发条件、输入、输出、状态变化或异常分支。
- 不清楚期望的新逻辑与现有逻辑的差异。
- 不清楚哪些现有逻辑必须保持不变。
- 不清楚技术方案文件、代码仓库、模块、关键词或相关上下文位置。
- 不清楚是否允许进入开发 Plan 或是否允许落库实现。

澄清时先用 2 到 5 句话复述当前理解，例如“我理解这次是要……，预期影响……，不希望改变……”，再提出需要用户确认的问题。澄清问题应尽量少，一轮最多 5 个；优先用选择题或要求用户补充具体文件、入口、行为。澄清完成前，不生成实现流程图、不生成 Mermaid、不调用生图、不制定开发 Plan、不改代码。

## 旧代码改动约束

在进入实现流程评审前，先判断本次是否需要改动旧代码。只要涉及旧代码，必须遵守：

- 以旧代码的目录、命名、分层、错误处理、日志、注释和测试风格为主，不强行引入新的个人风格。
- 禁止随意新增封装、抽象、工具层或中间适配层；只有旧代码已有相同模式，或能显著降低真实重复和复杂度时才允许。
- 禁止为了新需求顺手改写、重排或优化原有逻辑；除非旧逻辑必须变化，否则只在最小位置追加新分支、新字段、新校验或新转换。
- 对老代码优先采用追加式改动，尽量把新增逻辑挂在现有入口、现有流程和现有辅助函数附近，保持原有调用链和行为不变。
- 如果确实必须修改旧逻辑，必须在实现流程评审稿和开发 Plan 中说明原因、影响范围和为什么无法用追加方式完成。
...
- 图片中的流程澄清、逻辑动作、分支标签、分组标题和实现诠释必须以中文为主；英文只用于必要的代码标识、短文件名、方法名、字段名或协议类型。
- 图片不要把英文代码词汇当作主要说明文本；如果代码中是英文命名，应翻译成中文逻辑动作后再少量保留关键标识作为辅助定位，例如 `校验 tenantIDs 数量`、`组装 response map`。
...
- 涉及老代码时优先追加，不随意改写旧逻辑或新增封装；必须修改旧逻辑时先解释原因和影响范围。
