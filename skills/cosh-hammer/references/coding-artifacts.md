...
- `checkpoints/<task-id>.json`：详细任务的验收证据、staged files、snapshot SHA 和 commit SHA。`blocked` checkpoint 不含 commit。
...
每个 passed 任务由 `complete-subtask` 直接创建独立 commit/checkpoint，不接受外部 commit SHA。只有全部任务与 Git 历史一致且 HEAD 等于最后 checkpoint 时，`complete-coding` 才生成全局 handoff。中间父任务不产生 handoff或释放所有权。
