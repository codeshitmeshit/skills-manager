...
观察板固定监听 `127.0.0.1:57171`，使用系统默认浏览器打开 `http://127.0.0.1:57171/?work=<work-id>`。端口被非当前实例占用时停止，不得换端口。
...
CloudDev 场景仍使用固定一对一 SSH 转发：本机 `127.0.0.1:57171` 到远端 `127.0.0.1:57171`。远端不要使用 `--open`；隧道建立后在本机系统浏览器打开固定 URL。
