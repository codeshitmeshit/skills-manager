...
description: 根据技术笔记、操作步骤或学习资料生成静态 HTML 教程，也支持使用 Subagent 逆向分析完整仓库，解释架构、Happy Path、具体执行链路和真实 Git 演进。用户要求教程、指南、项目手册、陌生仓库走读或项目逆向教程时使用，统一维护 .cosh-docs/guide.html、learn/ 页面和 asset/ 资源。
---
...
- 普通教程：技术笔记、操作指南、部署与排障说明，直接执行下方默认工作流；不强制多 Agent、Git 历史分析或最低页数。
- 完整仓库逆向：先完整读取 [项目逆向工作流](references/project-reverse-workflow.md)，再开展分析。必须实际使用 Subagent 分工、证据交叉验证与质量闸门，按 70% 当前代码、30% 真实历史的认知重心组织内容；无历史证据不得编造。主 Agent 最后复用下方工作流与模板生成站点。
- 仅调整已有页面样式或链接：只改目标页面，不重新触发全仓库逆向。
...
- 每个最终 HTML 页面必须内联关键 CSS：`guide.html` 和 `.cosh-docs/learn/*.html` 都必须包含可独立渲染核心布局的 `<style>`。可以同时保留外部 CSS 文件作为复用或覆盖，但外部 CSS 不能是页面样式的唯一来源。
- 从模板生成页面时，默认保留模板内联 `<style>` 并替换内容；不得把模板样式全部抽成 `.css` 外链，除非同时把同等关键样式内联回每个 HTML 页面。
...
- 当教程来自项目逆向、源码走读、架构拆解或执行链路分析时，详情页必须保留高信息密度区块：目标元信息、核心判断、详细链路、数据流与分支、关键代码摘录、自己验证与调试入口、证据清单、关键概念补充、质量闸门与风险。不要把这些页面简化成普通“目标 + 步骤 + 代码 + 下一步”的低密度教程。
...
- 顶部必须给出入口、触发条件、终止点、页面置信度或等价元信息。
- 必须有 3 到 5 条核心判断或阅读提示，帮助读者先建立判断框架。
- 必须有详细链路，默认不少于 6 个连续步骤；项目本身不足 6 步时说明限制并覆盖全部真实步骤。
- 必须有数据流与分支表，说明输入、中间状态、输出、分支条件、异常或兜底行为。
- 必须有至少 2 段关键代码摘录，复杂链路建议 3 到 5 段，每段都解释其在链路中的作用。
- 必须有自己验证与调试入口，说明最小命令或触发方式、前置条件、样例输入、预期观察结果、常见失败检查点、断点或日志观察点。
- 必须有证据清单，映射结论到文件、函数、配置、命令输出、commit 或 diff。
- 必须有风险或置信边界说明，明确哪些内容未验证。
...
- `guide.html` 和所有 `.cosh-docs/learn/*.html` 都包含内联 `<style>`，即使外部 CSS 加载失败也能显示核心布局、卡片、标签、代码块、表格和步骤块样式。
- 图片路径正确。
- 移动端没有明显错位或文字溢出。
