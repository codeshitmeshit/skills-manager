...
自动接入最终失败时允许通用规则降级，但必须记录安装或更新命令、退出状态、脱敏错误、最终原因和规则版本。未真实尝试或证据不完整时保持 `fail closed`，不能静默跳过。
...
AI-Spec 不可访问时仍执行三路完整评审，并在每路结论中标记“通用规则降级”。通用规则覆盖稳定性 ST1–ST9、安全性 SEC1–SEC8 和可行性 F1–F8。恢复 AI-Spec 后，在进入规格阶段前补做知识门禁和受影响评审。
