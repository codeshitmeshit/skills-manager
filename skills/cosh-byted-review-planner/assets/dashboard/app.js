...
const sampleData = {
  ...
  documents: [
    { category: "source", label: "技术文档", path: ".byted-dev/work/optimize-order-risk-check/technical-design.md", content: "# 技术方案\n\n优化内部重试风险判断。" },
    { category: "spec", label: "规格", path: "docs/byted-dev/specs/2026-08-02-optimize-order-risk-check-design.md", content: "# 规格\n\n当前为示例内容。" },
    { category: "plan", label: "计划", path: "docs/byted-dev/plans/2026-08-02-optimize-order-risk-check.md", content: "# Plan\n\n### Task 1: 增加内部重试判断" },
  ],
};
...
function renderHeader(data) {
  ...
  return `<header class="top"><div>${renderWorkSwitcher(data)}<p class="eyebrow">BYTEDANCE · 原生研发</p><h1>${escapeHtml(data.work)}</h1></div><div class="connection ${stale ? "stale" : connectionState}"><strong>${connection}</strong><span>源版本 v${escapeHtml(data.source?.version || "-")}</span><span>${escapeHtml(data.read_at || "")}</span></div></header>${renderNavigation(data)}${issue ? `<p class="projection-warning" role="status">实时数据暂不可用，正在展示最后有效快照：${escapeHtml(issue)}</p>` : ""}`;
}
...
function modeControl(data) {
  const dirty = Boolean(data.current_task?.changed_files?.length || data.current_task?.staged_files?.length || data.scope_violation_files?.length);
  const disabled = data.stale || dirty ? "disabled" : "";
  return `<div class="mode-control"><div><p class="eyebrow">推进方式</p><strong>${data.mode === "continuous" ? "连续推进" : "逐一任务校验"}</strong><p>${dirty ? "当前任务有未提交改动，完成后可切换模式。" : "可在任务边界切换模式。"} 当前授权：${data.authorized_task ? `Task ${escapeHtml(data.authorized_task)}` : "暂无"}。UT / CR 时机由下方验证策略决定。</p></div><div class="segmented"><button data-mode="single" ${disabled} class="${data.mode === "single" ? "active" : ""}">逐一任务校验</button><button data-mode="continuous" ${disabled} class="${data.mode === "continuous" ? "active" : ""}">连续推进</button></div></div>`;
}
...
async function updateControl(data, action, extra = {}) {
  ...
  try {
    const response = await fetch("/api/control", {
      ...
      body: JSON.stringify({ action, origin: "dashboard", work: data.work, expected_version: data.version, idempotency_key: idempotencyKey(), ...extra }),
    });
