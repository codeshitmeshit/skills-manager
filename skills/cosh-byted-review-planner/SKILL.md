---
name: cosh-byted-review-planner
description: 字节专属的强门禁研发流程。用户提供技术方案并要求进行稳定性、安全性、可行性评审，使用 AI-Spec 与 CodeGraph 分析代码，以内置 Prompt 生成规格和计划，再按最小修改面逐任务编码、执行 BITS 远程 UT、CR、规范提交、push 或查看实时观察板时使用。
---
...
把技术方案转成可评审、可定位、可实施和可验证的字节研发任务。规格与计划由本 skill 的确定性 Prompt 直接生成，不依赖外部规划框架。所有入口共用同一套后端状态和证据校验；网站只提供实时观察与可选控制，不是唯一入口。

## 必读资源

- 开始流程或判断阶段前，完整读取 [`references/byted-workflow.md`](references/byted-workflow.md)。
- 生成或修正规格、计划前，完整读取 [`references/spec-plan-prompts.md`](references/spec-plan-prompts.md)。
...
- 实施阶段修改规格前，完整读取 [`references/implementation-spec-amendments.md`](references/implementation-spec-amendments.md)。
...
```bash
python3 <skill-root>/scripts/serve_byted_dashboard.py \
  --project <目标仓库> \
  --work <work-id> \
  --port 57171 \
  --open
...
- 本流程与 Hammer 在整个研发任务内硬互斥。选择本流程后，只能使用本 skill 的原生流程；禁止调用 `hammer`、`hammer-design`、`hammer-plan`、`hammer-execute`、`hammer-report`、`hammer-sync`、`hammer-lite`、`hammer-knowledge` 及其他 Hammer 流程，也不得读取、继承或写入 Hammer 状态和产物。
...
- 评审闭环通过后，才按内置规格 Prompt 生成规格；精确定位通过后，才按内置计划 Prompt 生成计划。Prompt 输入、输出结构与拒绝条件均为本 skill 的门禁契约。
...
- 全量远程 UT 与最终 CR 必须关联当前 HEAD 并通过，才允许普通 `git push`。
...
- 推进模式与验证策略独立：`single` 只要求逐任务授权；是否逐任务 UT/CR 取决于 `per_task`。模式只允许在没有未提交代码的任务边界切换。
- 仪表盘和聊天统一使用观察板 `/api/control`；聊天切换和推进必须携带 `origin=chat`、当轮 `message_id`、规范化 `user_intent`，保留真实用户消息供核对。不得自行伪造消息或改写控制 JSON。控制字段是审计记录，不是对拥有本地文件写权限的 Agent 的身份隔离。
- 连续模式每完成一个 Task，调用 `action=advance-auto`、`origin=automation`，仍提供当前 `expected_version`、`expected_task`、唯一 `idempotency_key` 和中文提交信息。页面不自动编写代码；主执行者负责消费模式并逐任务执行。
- 多个 Task 共用文件时，提交前必须有绑定当前快照的 `scope-taskN.json`，审查本次 diff 的符号、变量、接口是否仅属当前 Task。文件级检查无法替代此审查。
...
- 已进入实施阶段后修改规格，默认自动写成当前 `authorized_task` 的附加修正并覆盖该 Task 的文件、接口、步骤和验收条件；保留原规格、定位和计划基线。只有用户当轮明确要求重新生成规格或计划时才重建，Agent 不得因规格 SHA 变化自行重建。
- 同一开发任务不得在本流程与 Hammer 之间切换。检测到 Hammer 引擎、调用记录、状态或产物时立即阻塞；如需改用 Hammer，先终止并归档当前任务，再建立完全独立的新任务。
...
8. 用户确认评审闭环后，严格执行内置“规格生成 Prompt”，写入 `docs/byted-dev/specs/`，校验必需章节、事实引用和验收标准，再完成书面确认。
9. 再次校验文件、符号、变量和接口定位；规格与代码事实冲突时退回评审。
10. 严格执行内置“实施计划生成 Prompt”，写入 `docs/byted-dev/plans/`；每个实施子任务声明文件、符号、变量、接口、以 `Test:` 标注的测试交付物、验证与完成条件，并把测试编写放在实现步骤之前。
...
- 使用 `.byted-dev/work/<work-id>/` 保存字节门禁和证据旁路状态。
- 使用 `.byted-dev/work/<work-id>/dashboard-state.json` 原子保存最后有效观察板快照；该文件不属于门禁证据，也不参与状态版本计算。实时投影失败或服务重启恢复时只用于展示，并将所有推进动作禁用到真实证据重新可读。
- 使用 `docs/byted-dev/specs/`、`docs/byted-dev/plans/` 读取规格与计划；使用 `.byted-dev/work/<work-id>/` 读取任务进度和门禁证据。
- 入口阶段使用 [`scripts/serve_byted_dashboard.py`](scripts/serve_byted_dashboard.py) 自动启动并打开系统默认浏览器；用户关闭页面时仍按同一流程执行。
...
- 确认实施期规格变更绑定当前 Task、原规格与原计划哈希，且没有未经用户明确授权重建规格或计划。
