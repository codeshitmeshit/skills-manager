...
仅当原生规格、定位和计划已经通过，且存在当前已授权实施 Task 时使用。实现过程中修改 `docs/byted-dev/specs/` 下的当前规格，默认把变更同步为当前 Task 的附加修正，不重新执行规格与计划生成 Prompt，不重写原规格证据、定位证据或计划。

只有用户在当前轮明确要求“重新生成规格/计划”时，才放弃附加修正路径并执行完整重建。历史上的“继续”“调整一下”或规格文件发生变化都不构成重建授权。
...
5. 将规格文件与当前 Task 代码一起暂存和提交。原 `spec.json`、`location.json`、`plan.json` 及计划正文保持不变。
...
```json
{
  "status": "passed",
  "task": 2,
  "spec_path": "docs/byted-dev/specs/YYYY-MM-DD-<work>-design.md",
  "base_spec_sha256": "<上一有效规格 SHA-256>",
  "amended_spec_sha256": "<当前规格 SHA-256>",
  "base_plan_sha256": "<原计划 SHA-256>",
  "summary": "<本轮规格修正摘要>",
