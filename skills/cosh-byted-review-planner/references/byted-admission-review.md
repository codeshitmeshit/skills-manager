...
`sources` 的路径相对目标仓库，必须指向实际加载的非空文件，不能越界。七个角色为 `stability_skill`、`stability_spec`、`security_skill`、`security_spec`、`general_components`、`lark_general_knowledge`、`code_review`。`reviewer_sources` 把前两项映射给 `stability`，中间两项映射给 `security`，最后三项映射给 `feasibility`；`loaded_at` 记录读取时间。后端每次投影重新核对文件和 SHA-256。
