...
`docs/byted-dev/` 保存规格和计划；`.byted-dev/work/<work-id>/` 保存字节知识、评审、执行进度和控制证据。页面、API 和自然语言只调用同一个状态投影与转换验证器。

所有状态仅允许 `pending`、`running`、`blocked`、`passed`。前一阶段不是当前版本的 `passed` 时，后一阶段不得启动。文件存在、口头确认、隐藏按钮或旧版本结论都不是通过证据。

## 研发引擎互斥

创建开发任务时在 `workflow.json` 固定记录 `engine: byted-native`。该字段缺失、被改为 `hammer` 或其他值时，从技术文档入口直接阻塞，后续评审、规格、计划、实施、测试和 push 均不得继续。
...
必须按首次出现顺序推进：技术文档 → AI-Spec 知识门禁 → CodeGraph 事实扫描 → 稳定性评审 → 安全性评审 → 可行性评审 → 评审闭环 → 原生规格 → 精确定位 → 原生计划 → 实施子任务 → 远程 UT → 最终 CR → push → 本地归档。
...
```text
docs/byted-dev/specs/YYYY-MM-DD-<work>-design.md
docs/byted-dev/plans/YYYY-MM-DD-<work>.md
.byted-dev/work/<work>/task-progress.json
```
...
规格、定位和计划通过后，规格文件变更不再默认回退并重建全部产物。存在当前授权 Task 时，按 [`implementation-spec-amendments.md`](implementation-spec-amendments.md) 写入 `evidence/spec-amendment-task<N>.json`，以完整 `task_override` 替代原计划中当前 Task 的执行契约；原规格证据、定位证据、计划文件及计划证据保持不变。

附加修正进入当前 Task 快照，使旧远程 UT 和 CR 失效。规格文件必须随当前 Task 完整暂存和提交。已完成 Task 的修正只作为有效规格链历史，不得覆盖后续 Task。只有用户当轮明确要求重新生成规格或计划，或变更无法收敛到当前 Task 且用户确认重建时，才使原规格、定位和计划失效。
...
通过同一 `/api/control` 调用 `advance-auto`（`origin=automation`），由主执行者完成逐任务提交；单独模式拒绝此动作。用户模式切换只在工作区没有未提交代码时生效。聊天控制声明 `origin=chat`、真实的当轮 `message_id`，并使用 `user_intent=切换为连续推进`、`切换为单独推进` 或 `推进下一个任务`。服务把成功操作写入 `control.json.authorization_history`；这些字段用于审计，不能证明本地调用者身份。

同文件跨任务时，提交前写入 `evidence/scope-taskN.json`，包含 `status=passed`、`task=N`、当前任务快照 `code_sha`、`reviewer`、非空 `review_summary` 和 `out_of_scope_changes=[]`。Reviewer 必须对照当前任务与其他任务的符号、变量、接口审查实际 diff，发现提前实现后续任务时不得写通过。代码变化后重审。
...
```text
.byted-dev/archive/<work-id>/<completed-at>-retrospective.md
```
