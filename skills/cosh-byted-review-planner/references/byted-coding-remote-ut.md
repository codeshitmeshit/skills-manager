...
逐任务 `remote-ut-taskN.json` 必须记录 `status`、当前快照 `code_sha`、`prepare_only=false`、`exit_code=0`、非空 `result_dir` 和 `scope`，以及上述 `meta.remote` 与 `summary` 原始判定字段。`cr-taskN.json` 必须包含 `status`、当前快照 `code_sha`、非空 `reviewer`、`review_summary` 与 `blocking_findings=[]`。页面与推进接口使用相同验证器，禁止仅凭手写 `status=passed` 放行。
