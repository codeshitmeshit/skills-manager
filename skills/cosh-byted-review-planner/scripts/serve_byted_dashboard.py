#!/usr/bin/env python3
"""Serve the ByteDance-native workflow projection over HTTP and SSE."""
...
LOGGER = logging.getLogger("byted-review-dashboard")
...
def _documents(project_root: Path, work_id: str) -> list[dict[str, str]]:
    ...
    for category, root in (
        ("spec", project_root / "docs" / "byted-dev" / "specs"),
        ("plan", project_root / "docs" / "byted-dev" / "plans"),
    ):
        if not root.is_dir():
            continue
...
def _set_mode(
    ...
    # 只在任务边界切换；当前任务已有代码时必须先完成或清理。
    if task_control._run_git(project_root, ["rev-parse", "--git-dir"], check=False).returncode == 0:
        dirty = sorted(set(task_control.changed_paths(project_root)) | set(task_control._staged_paths(project_root)))
        if dirty:
            raise workflow_state.DashboardConflict("当前任务存在未提交改动，不能切换推进模式：" + ", ".join(dirty))
    work_dir = workflow_state.resolve_work(project_root, work_id)
...
def _apply_control_locked(
    ...
    origin = payload.get("origin", "dashboard")
    if origin not in {"dashboard", "chat", "automation"}:
        raise workflow_state.DashboardError("未知控制来源")
    if action in {"set-mode", "advance-next", "authorize-next"}:
        if origin == "automation":
            raise workflow_state.DashboardConflict("自动执行不能代替用户切换模式或授权任务")
        if origin == "chat":
            expected_intent = ("切换为连续推进" if payload.get("mode") == "continuous" else "切换为单独推进") if action == "set-mode" else "推进下一个任务"
            if payload.get("user_intent") != expected_intent or not payload.get("message_id"):
                raise workflow_state.DashboardConflict("聊天控制必须记录当轮明确意图与消息标识")
    if action == "set-mode":
    ...
    elif action in {"advance-next", "advance-auto"}:
        if action == "advance-auto" and origin != "automation":
            raise workflow_state.DashboardConflict("自动推进必须使用 automation 来源")
    ...
    control = _load_control(work_dir)
    history = control.setdefault("authorization_history", [])
    history.append({"action": action, "origin": origin, "message_id": payload.get("message_id"),
                    "user_intent": payload.get("user_intent"), "task": payload.get("expected_task"),
                    "mode": payload.get("mode"), "expected_version": expected_version,
                    "updated_at": _iso_now()})
    _atomic_write_json(work_dir / "control.json", control)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Serve the ByteDance R&D dashboard")
...
def main() -> None:
    ...
    print(f"字节研发观察板: {dashboard_url}", flush=True)
