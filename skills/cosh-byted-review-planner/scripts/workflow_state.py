#!/usr/bin/env python3
"""Project ByteDance-native artifacts into a fail-closed workflow."""
...
LOGGER = logging.getLogger("byted-native-workflow")
...
def resolve_work(project_root: Path, work_id: str | None) -> Path:
    works_root = project_root.resolve() / ".byted-dev" / "work"
    if not works_root.is_dir():
        raise DashboardError("未找到 .byted-dev/work")
...
def _project_knowledge_gate(
    ...
    # 来源相对目标仓库解析，防止用任意字符串冒充已加载知识。
    root = work_dir.parents[2]
    sources = evidence.get("sources")
    roles: set[str] = set()
    if not isinstance(sources, list):
        blockers.append("AI-Spec sources 必须是数组")
    else:
        for item in sources:
            try:
                if not isinstance(item, dict) or not isinstance(item.get("path"), str) or not item["path"].strip():
                    raise DashboardError("来源缺少路径")
                role = item.get("role")
                if not isinstance(role, str) or role in roles:
                    raise DashboardError("来源角色缺失或重复")
                roles.add(role)
                path = _safe_child(root, item["path"])
                if not path.is_file() or not path.stat().st_size:
                    raise DashboardError("来源文件不存在或为空")
                if _sha256_file(path) != item.get("sha256"):
                    raise DashboardError("来源 SHA-256 已变化")
            except (DashboardError, OSError) as error:
                blockers.append(f"AI-Spec 来源无效：{error}")
    required = set().union(*REQUIRED_KNOWLEDGE.values())
    if required - roles:
        blockers.append("AI-Spec 缺少来源：" + ", ".join(sorted(required - roles)))
    mapping = evidence.get("reviewer_sources")
    for reviewer, expected in REQUIRED_KNOWLEDGE.items():
        declared = mapping.get(reviewer) if isinstance(mapping, dict) else None
        if not isinstance(declared, list) or any(not isinstance(v, str) for v in declared) or not expected.issubset(declared):
            blockers.append(f"AI-Spec {reviewer} 来源映射不完整")
    if not evidence.get("loaded_at"):
        blockers.append("AI-Spec 缺少读取时间")
...
def _project_spec(
    ...
    try:
        ...
        return {}, _stage("blocked", [str(error)], fix="生成并确认原生规格")
    blockers: list[str] = []
    if closure_stage["status"] != "passed":
...
def _project_location(
    ...
    try:
        ...
        if spec_stage["status"] != "passed":
            return {}, _stage("pending", ["原生规格尚未通过"])
        return {}, _stage("blocked", [str(error)], fix="补齐变量级精确实施位置")
    blockers: list[str] = []
    if spec_stage["status"] != "passed":
        blockers.append("原生规格尚未通过，已有定位证据不可使用")
    if not _matches_source(evidence, source):
...
def _project_plan(
    ...
    try:
        ...
        return {}, _stage("blocked", [str(error)], fix="生成并确认原生实施计划")
    blockers: list[str] = []
    if location_stage["status"] != "passed":
...
def build_status(project_root: Path, work_id: str | None = None) -> dict[str, Any]:
    ...
    if engine != "byted-native":
        source_stage = _stage(
            "blocked",
            [
                *source_stage.get("blockers", []),
                "研发引擎互斥：workflow.json 必须固定为 engine=byted-native，禁止接入 Hammer",
            ],
            fix="终止并归档其他研发引擎任务后，建立独立的字节原生开发任务",
    ...
    if plan_stage["status"] != "passed":
        implementation_blockers.append("原生计划门禁尚未通过")
