#!/usr/bin/env python3
"""Parse ByteDance-native plans and advance one scoped implementation task."""
...
LOGGER = logging.getLogger("byted-review-task-control")
...
IGNORED_WORKFLOW_PATH_PREFIXES = (
    ".byted-dev/",
    "docs/byted-dev/specs/",
    "docs/byted-dev/plans/",
)
...
def parse_byted_plan(path: Path) -> list[dict[str, Any]]:
    try:
        lines = path.read_text(encoding="utf-8").splitlines()
    except OSError as error:
        raise TaskControlError(f"读取原生计划失败：{error}") from error
    ...
    for task in tasks:
        ...
        if not task["symbols"]:
            raise TaskControlError(f"Task {task['number']} 缺少 Symbols")
        if not task["variables"]:
            raise TaskControlError(f"Task {task['number']} 缺少 Variables")
        if not task["interfaces"]:
            raise TaskControlError(f"Task {task['number']} 缺少 Interfaces")
        if not task["acceptance_criteria"]:
            raise TaskControlError(f"Task {task['number']} 缺少 Acceptance")
        if not task["steps"]:
...
def find_plan(project_root: Path, work_id: str) -> Path:
    plan_root = project_root.resolve() / "docs" / "byted-dev" / "plans"
    candidates = sorted(plan_root.glob(f"*-{work_id}.md")) if plan_root.is_dir() else []
    if not candidates:
        raise TaskControlConflict(f"未找到开发任务 {work_id} 的原生计划")
    if len(candidates) > 1:
        raise TaskControlConflict(f"开发任务 {work_id} 存在多份原生计划")
    return candidates[0]
...
def validate_task_scope(project_root: Path, task: Mapping[str, Any]) -> list[str]:
    ...
    amendment = task.get("spec_amendment")
    if isinstance(amendment, Mapping) and amendment.get("spec_path"):
        allowed.add(str(amendment["spec_path"]))
    # 元数据可在观察期间变化，但进入暂存区后同样必须属于本任务。
    return sorted(set(path for path in (*changed_paths(project_root), *_staged_paths(project_root)) if path not in allowed))
...
def _work_dir(project_root: Path, work_id: str) -> Path:
    ...
    root = project_root.resolve() / ".byted-dev" / "work"
...
def _evidence_matches_snapshot(
    ...
    return (
        ...
        and not _task_result_errors(prefix, evidence)
    )


def _task_result_errors(prefix: str, evidence: Mapping[str, Any]) -> list[str]:
    if prefix == "remote-ut":
        meta = evidence.get("meta")
        summary = evidence.get("summary")
        if (evidence.get("prepare_only") is not False
            or evidence.get("exit_code") != 0
            or not evidence.get("result_dir") or not evidence.get("scope")
            or not isinstance(meta, dict) or not isinstance(meta.get("remote"), dict)
            or meta["remote"].get("run_success") is not True
            or not isinstance(summary, dict) or summary.get("has_failures") is not False
            or summary.get("failed_packages") != 0 or summary.get("failed_tests") != 0):
            return ["远程 UT 执行、范围或报告证据未通过"]
    elif (evidence.get("blocking_findings") != []
          or not evidence.get("reviewer") or not evidence.get("review_summary")):
        return ["CR 结论缺失或仍有阻塞问题"]
    return []


def _shared_scope_errors(work_dir: Path, task: Mapping[str, Any], tasks: list[dict[str, Any]], snapshot: str) -> list[str]:
    shared = set(task["allowed_files"]) & {
        path for other in tasks if other["number"] != task["number"]
        for path in other["allowed_files"]
    }
    if not shared:
        return []
    try:
        evidence = _read_json(work_dir / "evidence" / f"scope-task{task['number']}.json")
        if (evidence.get("status") != "passed" or evidence.get("code_sha") != snapshot
            or evidence.get("task") != task["number"] or evidence.get("out_of_scope_changes") != []
            or not evidence.get("reviewer") or not evidence.get("review_summary")):
            raise TaskControlConflict("共享文件范围审查缺失、过期或未通过")
    except TaskControlError as error:
        return [f"同文件跨 Task 必须先完成当前 diff 的符号、变量和接口范围审查：{error}"]
    return []
...
def project_task_status(project_root: Path, work_id: str) -> dict[str, Any]:
    ...
    tasks = parse_byted_plan(find_plan(project_root, work_id))
    ...
    if current is not None:
        ...
        scope_review_errors = _shared_scope_errors(work_dir, current, tasks, snapshot_sha)
        advance_blockers.extend(scope_review_errors)
        if not staged:
...
def _validate_task_evidence(
    ...
    for label, prefix in (("远程 UT", "remote-ut"), ("CR", "cr")):
        ...
        errors = _task_result_errors(prefix, evidence)
        if errors:
            raise TaskControlConflict("；".join(errors))
...
def advance_task(
    ...
    *,
    ...
    workflow = _read_json(work_dir / "workflow.json")
    mode = workflow.get("mode", "single")
    if automatic and mode != "continuous":
        raise TaskControlConflict("单独推进模式禁止自动推进")
    if not automatic and mode != "single":
    ...
    if current is None:
        raise TaskControlConflict("所有实施子任务已经完成")
    if _authorized_task(work_dir, mode, completed, current["number"]) != current["number"]:
        raise TaskControlConflict(
    ...
    scope_errors = _shared_scope_errors(work_dir, current, tasks, snapshot_sha)
    if scope_errors:
        raise TaskControlConflict("；".join(scope_errors))
