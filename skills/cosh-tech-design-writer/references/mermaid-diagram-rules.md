...
- `三、方案总览：一张图看懂整体设计`：系统模块交互、核心调用链、整体架构逻辑。
- `四、详细设计`：每条核心路径的调用顺序、请求响应链路、数据读写和模块协作。
- `六、稳定性与安全 Q&A`：失败重试、降级、补偿、回滚、数据一致性和安全链路。
- `七、测试用例与上线方案`：灰度、切流、回滚流程和容量路径。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| DIAG-001 | 主 Agent | 三、方案总览：一张图看懂整体设计 | P2 中 | 缺少调用逻辑图 | 未提供核心调用方、被调方和调用顺序 | 补充调用入口、核心服务、下游依赖、同步/异步关系 | 否 |
```
