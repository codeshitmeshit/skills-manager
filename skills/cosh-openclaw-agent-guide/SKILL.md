...
description: Virtual Office 中任意 agent、CLI 或调用方已决定通过 VO 调用 OpenClaw，但需要选择或理解 OpenClaw 内部 agent 时使用；提供 OpenClaw agent 的角色说明、agentId、适用场景、输入输出和兜底调用规则。
cli_scope:
  - vo
  - openclaw
  - hermes
...
帮助 Virtual Office 中的任意 agent、CLI 或调用方在调用 OpenClaw 时选择正确的 OpenClaw 内部 agent。

本指南不是判断“是否应该调用 OpenClaw”的总开关，也不是普通 skill 路由器。它只在调用方已经决定通过 Virtual Office 调用 OpenClaw 后使用，用来理解 OpenClaw 内部各 agent 的角色、适用场景、调用输入、输出期望和优先级关系。

## 核心规则

调用方通过 VO 调用 OpenClaw 时必须明确指定目标 agent：

1. 如果调用方能确定最合适的 OpenClaw agent，直接使用该 agent 的 `agentId`。
2. 如果调用方不确定应该调用哪个 agent，指定 `main`。
...
`main` 是固定兜底协调 agent。调用方不确定时，不应自行猜测相近 agent，也不应停止在选择阶段；应把任务交给 `main`，由 `main` 决定后续处理方式。

## 什么时候使用本指南

当以下条件同时成立时，VO 内调用方应使用本指南：

1. 调用方已经决定通过 VO 调用 OpenClaw。
2. 调用方需要选择 OpenClaw 内部 agent，或需要理解某个 OpenClaw agent 的职责。
3. 调用方需要知道调用某个 agent 前应提供什么信息，以及调用后应期待什么结果。

典型场景：

1. 用户请求需要交给 OpenClaw 内部某个专业 agent。
2. VO 内某个 agent、CLI 或调用方需要在多个 OpenClaw agent 之间做选择。
3. 调用方不确定目标 agent，需要确认是否回退到 `main`。
4. OpenClaw agent 清单更新后，VO 内调用方需要按最新角色说明进行调用。

## 什么时候不要使用本指南

当以下任一条件成立时，不使用本指南：

1. 调用方还没有决定是否调用 OpenClaw。
2. 用户只是要求选择普通 skill，而不是选择 OpenClaw 内部 agent。
3. 用户已经明确要求使用某个非 OpenClaw skill。
4. 任务可以由当前调用方直接完成，不需要进入 OpenClaw。
5. 任务需要更新本指南本身；此时应按用户要求直接维护本 skill，而不是把它当作调用依据。

## 调用决策流程

调用方通过 VO 调用 OpenClaw 前，按以下流程选择 agent：
...
以下清单由 OpenClaw 在本 skill 创建后维护和更新。VO 内调用方在选择具体 OpenClaw agent 时，应优先参考本节。

### main

- agentId：`main`
- 角色：固定兜底协调 agent。
- 适用场景：调用方不确定应该调用哪个 agent；目标 agent 不在可用清单里；任务跨多个 agent 角色；需要 OpenClaw 内部继续判断或分派。
- 不适用场景：调用方已能明确选择某个可用专业 agent，且任务不需要协调。
- 输入要求：用户原始目标、调用方的任务理解、不确定点、候选 agent 或原本想指定的 agent、希望 main 判断或分派的事项。
...
OpenClaw 后续补充 agent 时，必须使用以下结构。每个 agent 条目都必须包含 `agentId`，VO 内调用方不应根据名称猜测调用目标。
...
调用方明确指定 OpenClaw agent 时，任务上下文应包含：
...
回退原因：
<为什么调用方无法明确指定专业 agent，或为什么目标 agent 不可用>
